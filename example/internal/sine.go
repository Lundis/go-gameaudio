...
func PlaySineWave(context *oto.Context, freq float64, duration time.Duration) *oto.Player {
	p := context.NewPlayer(GenerateSineWave(freq, duration), 1, oto.ChannelIdDefault)
	p.Play()
	return p
}
